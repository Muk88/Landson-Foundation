...
const socialLinks = [
    ...
    },
    {
        ...
        icon: (
            <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668 -0.014 4.948 -0.072 4.354 -0.2 6.782 -2.618 6.979 -6.98.059-1.28.073-1.689.073-4.948 0-3.259 -0.014-3.667 -0.072-4.947 -0.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/>
            </svg>
        ),
    },
    {
        ...
        <footer className="relative bg-gray-950 text-white overflow-hidden">
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-0">
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-x-8 gap-y-12 mb-16">
                    ...
                    {/* Brand column — spans full width on mobile/tablet, 2 cols on desktop */}
                    <div className="col-span-2 md:col-span-3 lg:col-span-2 space-y-7">
                        ...
                        <div className="space-y-3 pt-1">
                            {contactInfo?.email && (
                                <a href={`mailto:${contactInfo.email}`} className="flex items-center gap-3 text-gray-400 hover:text-white transition-colors group">
                            ...
                            {contactInfo?.phone && (
                                <a href={`tel:${contactInfo.phone}`} className="flex items-center gap-3 text-gray-400 hover:text-white transition-colors group">
                                    <div className="w-7 h-7 rounded-lg bg-white/5 flex items-center justify-center flex-shrink-0 group-hover:bg-brand-red transition-colors">
                                        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            {contactInfo?.address && (
                                <div className="flex items-center gap-3 text-gray-400">
                                    ...
                                    <span className="font-body text-sm whitespace-pre-wrap">{contactInfo.address}</span>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="space-y-6">
                        <h3 className="font-display font-bold text-[10px] uppercase tracking-[0.25em] text-white/40 flex items-center gap-2">
                        ...
                        <ul className="space-y-3.5">
                            {footerLinks.organization.map((link) => (
                                <li key={link.label}>
                                    <Link
                                        href={link.href}
                                        className="font-body text-gray-400 hover:text-white text-sm font-semibold transition-all duration-200 flex items-center gap-2 group"
                                    >
                ...
                <div className="border-t border-white/10 py-7 flex flex-col md:flex-row justify-between items-center gap-4 text-gray-500 text-xs">
                    <p className="font-body">© {currentYear} <span className="text-white font-bold">Landson Foundation</span>. All rights reserved.</p>
                    <div className="flex items-center gap-6 font-body">
                        <Link href="/privacy" className="hover:text-brand-green transition-colors font-semibold">Privacy Policy</Link>
                        <span className="w-px h-3 bg-white/20"></span>
                        <Link href="/terms" className="hover:text-brand-red transition-colors font-semibold">Terms of Service</Link>
                    </div>
                    <p className="font-body">Crafted with Passion by <a href="https://ascendstratus.com/" target="_blank" rel="noopener noreferrer" className="text-white hover:text-brand-green transition-colors font-bold underline decoration-brand-green/30 underline-offset-4">Ascend Stratus</a></p>
                </div>
            </div>
        </footer>
    )

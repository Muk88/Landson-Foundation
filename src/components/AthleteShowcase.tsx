    ...
    return (
        <section className="bg-gray-50 py-16 md:py-24">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="text-center max-w-3xl mx-auto mb-12 space-y-5">
                    <AnimateOnScroll variant="fade-up" delay={0}>
                        <span className="inline-block px-4 py-1.5 bg-brand-red-soft rounded-full">
                            <span className="label-text text-brand-red">Our Athletes</span>
                        </span>
                    </AnimateOnScroll>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    {athletes.map((athlete, index) => (
                        <AnimateOnScroll
                            ...
                            easing="spring"
                        >
                            <div className="group relative h-[500px] rounded-3xl overflow-hidden shadow-lg transition-all duration-500 hover:-translate-y-4 hover:shadow-2xl">
                                <Image
                                    src={athlete.image}
                                    alt={athlete.name}
                                    fill
                                ...
                                />
                                <div className="absolute inset-0 bg-gradient-to-t from-brand-green/90 via-brand-green/20 to-transparent opacity-80 group-hover:opacity-95 transition-opacity duration-500" />
                                <div className="absolute inset-0 p-8 flex flex-col justify-end text-white transform transition-transform duration-500 group-hover:translate-y-[-10px]">
                                    <span className="font-display font-black text-2xl uppercase tracking-[0.06em] mb-2">{athlete.name}</span>
                                    <p className="font-body text-white/80 text-sm leading-relaxed mb-6 opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-100">
                                        {athlete.tagline}
                                    </p>
                                    <div className="pt-6 border-t border-white/20 flex items-center justify-between">
                                        <div>
                                            <span className="stat-number block text-3xl text-brand-red leading-none">{athlete.stat}</span>
                                            <span className="label-text text-white/60 mt-1 block">{athlete.statLabel}</span>
                                        </div>
                ...
                {/* CTA */}
                <AnimateOnScroll variant="fade-up" delay={200} className="mt-20 flex flex-col sm:flex-row items-center justify-center gap-6">
                    <Link href="/programs">
